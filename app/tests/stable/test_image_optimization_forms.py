...
@pytest.mark.django_db
def test_info_form_clean_image_without_new_upload(event, submission):
    # Setup submission with an existing image
    existing_image = _create_test_image()
    submission.image.save('test.png', existing_image)
...
@pytest.mark.django_db
def test_speaker_profile_form_clean_avatar_with_new_upload(user, settings):
    settings.IMAGE_DEFAULT_MAX_WIDTH = 1000
    settings.IMAGE_DEFAULT_MAX_HEIGHT = 1000
    
    profile, _ = SpeakerProfile.objects.get_or_create(user=user)
...
@pytest.mark.django_db
def test_speaker_profile_form_clean_avatar_without_new_upload(user):
    profile, _ = SpeakerProfile.objects.get_or_create(user=user)
    existing_avatar = _create_test_image()
    user.avatar.save('avatar.png', existing_avatar)
    ...
    assert cleaned_avatar == user.avatar
